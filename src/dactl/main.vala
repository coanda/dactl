internal class Dactl.Main : GLib.Object {
    ...
    private static void parse_local_args (ref unowned string[] args) {
        var opt_context = new OptionContext (Dactl.PACKAGE_NAME);
    ...
    private int run (string[] args) {
        debug (_("Dactl v%s starting..."), Dactl.PACKAGE_VERSION);
        app.launch (args);
    ...
    private static int main (string[] args) {
        ...
        Intl.bindtextdomain (Dactl.GETTEXT_PACKAGE, Dactl.LOCALEDIR);
        Intl.bind_textdomain_codeset (Dactl.GETTEXT_PACKAGE, "UTF-8");
        Intl.textdomain (Dactl.GETTEXT_PACKAGE);

        GLib.Environment.set_prgname (_(Dactl.PACKAGE_NAME));
        GLib.Environment.set_application_name (_(Dactl.PACKAGE_NAME));

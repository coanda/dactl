...
//    }
    ...
    public PIDBox (string pid_id, Dactl.ApplicationModel model) {
    ...
    private void connect_signals () {
        ...
        (pid_enable as Gtk.ToggleButton).toggled.connect (() => {
            ...
            if ((pid_enable as Gtk.ToggleButton).active) {
                ...
                sp_spin_button.sensitive = false;
                manual_adjustment.value = (((mv as Cld.DataSeries).channel) as ScalableChannel).scaled_value;
                if (pid.running) {
                    pid.running = false;
                    thread.join ();
                }
...
public class PID2Box : Gtk.Box {
    ...
    public PID2Box (string pid_id, Dactl.ApplicationModel model) {
        GLib.Object (orientation: Orientation.HORIZONTAL);
        spacing = 10;
        //this.pid = pid;
        this.pid_id = pid_id;
    ...
    private void create_widgets () {
        ...
        r2hbox.pack_start (new Gtk.Label ("Output\n[% of max.]"), true, false, 0);
        r2hbox.pack_start (manual_spin_button, false, false, 0);
        r2hbox.pack_start (new Gtk.Label ("PID_2 Setpoint\n[" + units + "]"), true, false, 0);
        r2hbox.pack_start (sp_spin_button, false, false, 0);
    ...
    private void connect_signals () {
        ...
        (pid_enable as Gtk.ToggleButton).toggled.connect (() => {
            ...
            if ((pid_enable as Gtk.ToggleButton).active) {
                ...
                manual_spin_button.sensitive = false;
                sp_spin_button.sensitive = true;
                sp_adjustment.value = (((pv as Cld.DataSeries).channel) as ScalableChannel).scaled_value;
                if (!pid.running) {
                    pid.calculate_preload_bias ();
                    try {
                        pid.running = true;

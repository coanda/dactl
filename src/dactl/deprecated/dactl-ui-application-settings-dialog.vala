...
public class Dactl.ApplicationSettingsDialog : Dialog {
    ...
    construct {
        builder = load_ui ("settings-dialog.ui");
    }

    public ApplicationSettingsDialog (Dactl.ApplicationModel model) {
        this.model = model;
    ...
    public ApplicationSettingsDialog.with_startup_tab_id (Dactl.ApplicationModel model, int tab_id) {
        this.model = model;
        this.tab_id = tab_id;
...
//
//        var traverse_box = new Box (Gtk.Orientation.VERTICAL, 10);
//
...
//    }
    ...
    private void channel_notebook_switch_page_cb (Gtk.Widget page, uint page_num) {
        ...
        switch (channel_settings_notebook_page_text) {
            case "Analog Input":
                ...
                channel = get_selected_channel ();
                update_channel_entry_text (channel);
                debug ("Analog Input tab selected. Channel: %s\n", channel.id);
                populate_coefficient_treeview (channel);
                break;
            case "Analog Output":
                (aochannel_treeview as TreeView).set_cursor (treepath, null, false);
                channel = get_selected_channel ();
                update_channel_entry_text (channel);
                debug ("Analog Output tab selected. Channel: %s\n", channel.id);
                populate_coefficient_treeview (channel);
                break;
            case "Digital Input":
                (dichannel_treeview as TreeView).set_cursor (treepath, null, false);
    ...
    private void populate_coefficient_treeview (Cld.Channel channel) {
...
//                    debug ("%s %.3f\n", (coefficient as Coefficient).id, (coefficient as Coefficient).value);
    ...
    private Cld.Channel? get_selected_channel () {
        string id;
        string text;
        int column = -1;
        Gtk.Widget current_page;
        ...
        current_page = (channel_settings_notebook as Notebook).get_nth_page (channel_settings_notebook_page_num);
        channel_settings_notebook_page_text = (channel_settings_notebook as Notebook).get_tab_label_text (current_page);
        debug ("get_selected_channel (): %s page: channel_settings_notebook_page_num: %d\n",
        ...
        return channel as Cld.Channel;
    }

    private void update_channel_entry_text (Cld.Channel channel) {
    ...
    private void module_cursor_changed_cb () {
        ...
        debug ("Module TreeView Selection :: Module ID: %s Module Type: %s\n", module.id, type);
        /* Empty display area */
        foreach (var child in (scrolledwindow_module_view1 as Gtk.Container).get_children ()) {
            (scrolledwindow_module_view1 as Gtk.Container).remove (child as Gtk.Widget);
        }
        foreach (var child in (scrolledwindow_module_view2 as Gtk.Container).get_children ()) {
            (scrolledwindow_module_view2 as Gtk.Container).remove (child as Gtk.Widget);
        }
        ...
        switch (type) {
            ...
            case ("CldVelmexModule"):
                add_serial_port_settings (module as Cld.Module);
                message ("Plugins need to provide their own settings dialog.");
                /*
                 *VelmexSettingsBox velmex_settings_box = new VelmexSettingsBox (module as Cld.Module);
            ...
            case ("CldParkerModule"):
                ...
                message ("Plugins need to provide their own settings dialog.");
                /*
                 *ParkerSettingsBox parker_settings_box = new ParkerSettingsBox (module as Cld.Module);
    ...
    private void add_serial_port_settings (Cld.Module module) {
        Cld.SerialPort port;
        SerialPortSettingsBox serial_port_settings_box;
        port = (module as Cld.Module).port as Cld.SerialPort;
    ...
    private void update_log_config () {
        ...
        debug ("log_rate: %.3f  get_value: %.3f\n", (log as Cld.Log).rate, (btn_log_rate as Gtk.SpinButton).get_value ());
    ...
    private void update_aichannel_config () {
        ...
        foreach (var coefficient in ((coefficient_treeview as CoefficientTreeView).coefficients.values)) {
            debug ("channel.id: %s calibration.id: %s coefficient.id %s\n", channel.id,
                   (channel as Cld.ScalableChannel).calibration.id, (coefficient as Cld.Coefficient).id);
            (channel as Cld.ScalableChannel).calibration.set_coefficient (
                (coefficient as Cld.Coefficient).id, (coefficient as Cld.Coefficient));
        }
         ...
         *(scrolledwindow_aichannel as Gtk.Widget).show_all ();
         */
    }
    ...
    private void update_aochannel_config () {
        ...
        (channel as Cld.Channel).desc = (entry_channel_desc as Gtk.Entry).text;
        (channel as Cld.Channel).num = int.parse ((entry_channel_num as Gtk.Entry).text);
        //debug ("Selected chan is: %s\n", channel.id);

        /* Apply new calibration. */
        foreach (var coefficient in ((coefficient_treeview as CoefficientTreeView).coefficients.values)) {
            debug ("channel.id: %s calibration.id: %s coefficient.id %s %.3f\n",
                   channel.id,
                   (channel as Cld.ScalableChannel).calibration.id,
                   (coefficient as Cld.Coefficient).id,
         ...
         *(scrolledwindow_aochannel as Gtk.Widget).show_all ();
         */
    }
    ...
    private void update_vchannel_config () {
         ...
         *(scrolledwindow_vchannel as Gtk.Widget).show_all ();
         */
    }
    ...
    private void update_dichannel_config () {
         ...
         *(scrolledwindow_dichannel as ScrolledWindow).add (dichannel_treeview);
         *(scrolledwindow_dichannel as Gtk.Widget).show_all ();
         */
    ...
    private void update_dochannel_config () {
         ...
         *(scrolledwindow_dochannel as ScrolledWindow).add (dochannel_treeview);
         *(scrolledwindow_dochannel as Gtk.Widget).show_all ();
         */

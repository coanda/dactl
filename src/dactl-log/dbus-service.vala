[DBus (name = "org.coanda.Dactl.Recorder")]
internal class Dactl.Recorder.DBusService : GLib.Object, Dactl.DBusInterface {

    private Dactl.Recorder.Main main;
    private uint name_id;
    private uint connection_id;

    public DBusService (Dactl.Recorder.Main main) {
        this.main = main;
    }

    internal void publish () {
        this.name_id = Bus.own_name (BusType.SESSION,
                                     Dactl.DBusInterface.SERVICE_NAME + ".Recorder",
        ...
        try {
            this.connection_id =
                connection.register_object (
                    Dactl.DBusInterface.OBJECT_PATH + "/Recorder", this
                );
        } catch (Error error) {

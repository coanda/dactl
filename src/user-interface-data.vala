...
public class UserInterfaceData : GLib.Object {
    ...
    private Gtk.Widget btn_save;
    private Gtk.Widget btn_pref;
    ...
    private Gee.List<PID2Box> pid_box_list = new Gee.ArrayList<PID2Box> ();
    ...
    construct {
        ...
        try {
            ...
            btn_save = builder.get_object ("btn_save") as Widget;
            /* Edit menu ------------------------------------------------------- */
    ...
    public UserInterfaceData (ApplicationData cb_data) {
        ...
        (main_window as Window).maximize ();
    ...
    private void connect_signals () {
        ...
        (btn_log as Gtk.ToggleToolButton).toggled.connect (() => {
            ...
            if (log != null) {
                ...
                if (Posix.access ((log as Cld.Log).path, mode) == 0) {
                    ...
                    Gtk.MessageDialog msg = new MessageDialog (null, DialogFlags.MODAL,
                                                            MessageType.ERROR,
                                                            ButtonsType.CANCEL,
                                                            "File access permission denied: %s\n",
                                                            (log as Cld.Log).path);
                    msg.secondary_text = "Use the Chooser to select a new directory.";
                    ...
                    var dialog = new LogSettingsDialog (log as Cld.Log);
                    while (!((dialog as LogSettingsDialog).done)) {
                        (dialog as Dialog).run ();
                        Cld.debug ("running ...\n");
                ...
                if (Posix.access ((log as Cld.Log).path, mode) == 0) {
                    ...
                    Gtk.MessageDialog msg = new MessageDialog (null, DialogFlags.MODAL,
                                                            ...
                                                            "File access permission denied: %s\n",
                                                            (log as Cld.Log).path);
                    msg.secondary_text = "No log file will be generated.";
                ...
                if ((btn_log as Gtk.ToggleToolButton).active && go) {
                ...
                /* Prompt user to edit the configuration file. */
                Gtk.MessageDialog msg = new MessageDialog (null, DialogFlags.MODAL,
                                                        MessageType.ERROR,
                                                        ButtonsType.CANCEL,
                                                        "No log configuration data: %s\n",
                                                        (log as Cld.Log).path);
                msg.secondary_text = "Edit the configuration XML file and restart the program.";
        ...
        var btn_pref = builder.get_object ("btn_pref");
        (btn_pref as Gtk.ToolButton).clicked.connect (mnu_item_edit_pref_activate_cb); // Reusing the callback that is in the menu items.

        var btn_chan = builder.get_object ("btn_chan");
        (btn_chan as Gtk.ToolButton).clicked.connect (mnu_item_edit_chan_activate_cb);
    ...
    private void add_chart_content () {
        ...
        (chart_scroll as Gtk.ScrolledWindow).min_content_width = 1000;
    ...
    private void add_control_content () {
        ...
        foreach (var pid in model.control_loops.values) {
            ...
            if (pid is Pid) {
                ...
                var pid_box = new PID2Box (pid.id, model);
                pid_box.settings_dialog = new PID2SettingsDialog (pid as Cld.Pid2, model.channels);
                pid_box_list.add (pid_box);
            }
        }

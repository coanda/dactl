...
public class ChartWidget : Gtk.Box {
    ...
    private Gtk.Widget chart_box;
    private Gtk.Widget lbl_title;
    private Gtk.Widget lbl_x_axis;
    private Gtk.Widget lbl_y_axis;
    private Gtk.Widget btn_settings;
    ...
    construct {
        ...
        try {
            ...
            chart_box = builder.get_object ("chart_box") as Gtk.Widget;
            lbl_title = builder.get_object ("lbl_title") as Gtk.Widget;
            lbl_x_axis = builder.get_object ("lbl_x_axis") as Gtk.Widget;
            lbl_y_axis = builder.get_object ("lbl_y_axis") as Gtk.Widget;
            btn_settings = builder.get_object ("btn_settings") as Gtk.Widget;
    ...
    public virtual void add_series (string id) {
        (chart_area as ChartArea).data_series.set (id, new Gee.ArrayList<Point> ());
    }

    public virtual void add_series_with_data (string id, Gee.List<Point> data) {
        (chart_area as ChartArea).data_series.set (id, data);
    }

    public virtual void add_point_to_series (string id, double x, double y) {
        var list = (chart_area as ChartArea).data_series.get (id);
        list.add (new Point (x, y));
    }

    public virtual void add_series_color (Gee.List<double?> color) {
        (chart_area as ChartArea).series_colors.add (color);
    }

    public virtual void select_series (string series_id) {
        (chart_area as ChartArea).selected_series = series_id;
    }
}
...
public class StripChartWidget : ChartWidget {
    ...
    public override void add_point_to_series (string id, double x, double y) {
        ...
        if (!(chart_area as ChartArea).data_series.has_key (id))
            return;
        ...
        if (list.size >= (x_axis_max / time_step) + 1) {
            for (has_next = iter.first (); has_next; has_next = iter.next ()) {
                var point = iter.get ();
                point.x -= time_step;
            }
        } else {
            has_next = iter.first ();
            for (has_next = iter.next (); has_next; has_next = iter.next ()) {
                var point = iter.get ();
...
public class ChartArea : Gtk.DrawingArea {
    ...
    public Gee.Map<string, Gee.List<Point>> data_series;
    ...
    construct {
        data_series = new Gee.TreeMap<string, Gee.List<Point>> ();
    ...
    public override bool draw (Cairo.Context cr) {
        ...
        foreach (var key in data_series.keys) {
            ...
            Gee.List<Point> subset = new Gee.ArrayList<Point> ();
    ...
    private void draw_line (Cairo.Context cr, Gee.List<Point> data) {
        var w = get_allocated_width ();
        var h = get_allocated_height ();
    ...
    private void draw_polyline (Cairo.Context cr, Gee.List<Point> data) {
        var w = get_allocated_width ();
        var h = get_allocated_height ();
        int i;
